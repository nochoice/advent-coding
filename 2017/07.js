...
findRoot = (flatStructure) => {
    let children = flatStructure.reduce((acc, node) => {
        return [...acc, ...node.children];
    }, []);

    let nodes = flatStructure.reduce((acc, node) => {
        return [...acc, node.name];
    }, []);

    let diff = nodes.filter(name => children.indexOf(name) === -1);
